...
Ultron.setAngle(270); Ultron.microMove(10);
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = Ultron.getX();
		int robotYLocation = Ultron.getY();
		
		// 6. Print the robotXLocation and robotYLocation variables to the console 
		System.out.println(" "+robotXLocation); System.out.println(" "+robotYLocation);
		// 7. If robot is at same location as the little girl
		//      --make a pop-up tell the robot where to go next
		if(robotXLocation == 720 && robotYLocation == 400) {JOptionPane.showMessageDialog(null, "The Girl says the parrot might know where the treasure is");}
		// 8. Give the user subsequent clues at different locations on the image
		// (pirate robot, swamp, parrots, etc.)
		if(robotXLocation == 620 && robotYLocation == 140) {JOptionPane.showMessageDialog(null, "The parrot says the treasure location is on a piece of paper in a bottle");}
		// 9.  If the robot is in the final location
		//     --call the treasureFound() method
		if(robotXLocation == 410 && robotYLocation == 510) {JOptionPane.showMessageDialog(null, "The bottle says the treasure is in the skulls nose");}
	if(robotXLocation == 250 && robotYLocation == 120) {JOptionPane.showMessageDialog(null, "Yay! You found the treasure!");}
	}
